...
type ApiResult = {
  ...
  name: string;
  readyDate: string;
  baseSummary: string; // ←「基本アドバイス」本文
  advice?: Advice;     // ←「AIからの追加提案」
};
...
export default function RipenessPage() {
  ...
  const [storage, setStorage] = useState<"room" | "cooldark" | "vegroom" | "fridge">("room");
  ...
  // 1) フルーツ一覧をAPIから取得（fruit_rules.jsonが全部出ます）
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  // UI: 見た目はシンプル（Tailwind前提）。必要に応じてお好みに調整OK
  return (
    <div className="mx-auto max-w-3xl p-4 md:p-6">
      ...
      <div className="space-y-6 bg-white rounded-xl shadow-sm border p-4 md:p-6">
        ...
        <div>
          <button
            ...
            className="inline-flex items-center justify-center rounded-lg bg-black text-white px-5 py-2.5 disabled:opacity-40"
          >
      ...
      {result && (
        <div className="mt-8 space-y-6">
          ...
          <p className="text-xs opacity-60">実際の環境で差があります。目安としてご活用ください。</p>
        </div>
      )}
    </div>
  );
